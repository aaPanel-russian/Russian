var site_table = bt_tools.table({
    ...
    minWidth:'1000px',
    autoHeight:true,
    default:lan.site.site_empty, // 数据为空时的默认提示
    ...
    column:[
        ...
        {fid:'ssl',title:'SSL',tips:'Deployment certificate',width:110,type:'text',template:function(row,index){
            ...
            return _ssl === -1?'<a class="btlink" href="javascript:;" style="color:orange;">'+lan.site.not_set+'</a>': (_ssl.endtime >= 0? '<a class="btlink" href="javascript:;" title="'+ _info +'">Expire: '+ _ssl.endtime +'days</a>':'<a style="color:red" href="javascript:;">Expired</a>');
    ...
    tootls:[{ // 按钮组
        ...
        positon:['left','top'],
        list:[
            {title: lan.site.site_add, active:true, event:function(ev){ site.add_site(function(){ 
                site_table.$refresh_table_list(true) });
                bt.set_cookie('site_type','-1');
            }},
            {title: lan.site.set_default_page, event:function(ev){ site.set_default_page() }},
            {title: lan.site.set_default_site, event:function(ev){ site.set_default_site() }},
            {title: lan.site.get_cli_version, event:function(ev){ site.get_cli_version()}},
            {title:'Category manager',group:true,init:function(className){
                bt.site.get_type(function(res){
                    ...
                    html += '<li role="separator" class="divider"></li><li><a href="javascript:;" data-id="type_sets">'+lan.site.type_sets+'</a></li>';
        ...
        type:'search',
        positon:['right','top'],
        placeholder:lan.site.search,
        searchParam:'search', //搜索请求字段，默认为 search
        value:'',// 当前内容,默认为空
    },{ // 批量操作
        type:'batch',//batch_btn
        positon:['left','bottom'],
        placeholder:'Select batch operation2',
        buttonValue:lan.site.execute,
        disabledSelectValue:lan.site.execute_warn,
        selectList:[
            {
                group:[{title:lan.site.enable_website,param:{status:1}},{title:lan.site.disable_website,param:{status:0}}],
        ...
        var add_web = bt_tools.form({
            ...
            form:[{
                    ...
                    help:{
                        ...
                        list:[lan.site.ssl_arecord],
                    },
                    group:[{
                        type:'checkbox',
                        name:'set_ssl',
                        label_tips:lan.site.set_ssl,
                        event:function(value,form,that,config,ev){
            ...
            var _this = this;
                ...
                bt_tools.table({
                    ...
                    column:[
                        ...
                        {title:lan.site.opt,width:80,type:'group',align:'right',group:[{
                            title:lan.site.del,
                            template:function(row,that){
                                return that.data.length === 1?'<span>'+lan.site.inoperable+'</span>':lan.site.del;
                            },
                            event:function(row,index,ev,key,that){
                    ...
                    tootls:[{ // 批量操作
                        ...
                        positon:['left','bottom'],
                        config:{
                            title: lan.site.delete_multiple,
        ...
        set_dirbind: function (web) {
            ...
            bt_tools.table({
                ...
                column:[
                    ...
                    {title:lan.site.opt,width:130,type:'group',align:'right',group:[{
                        title:lan.site.url_rewrite,
                        event:function(row,index,ev,key,that){
                        ...
                        title:lan.site.del,
                        event:function(row,index,ev,key,that){
                            bt.confirm({title:lan.site.sub_del + '【'+ row.path +'】' + lan.site.sub_del2, msg: lan.site.s_bin_del }, function () {
